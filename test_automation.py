...
def _block_external_redirects(page, target_url):
    """Intercept and block navigations away from the target domain."""
    target_domain = urlparse(target_url).netloc  # e.g. "www.pixelssuite.com"

    def handle_route(route):
        req_url = route.request.url
        if route.request.resource_type == "document":
            req_domain = urlparse(req_url).netloc
            # Block document navigations that leave the target domain entirely
            if req_domain and target_domain not in req_domain and req_domain not in target_domain:
                print(f"  [REDIRECT BLOCKED] -> {req_url[:80]}")
                try:
                    route.abort("aborted")
                except Exception:
                    pass
...
def _load_page(page, url):
    """Navigate to the translator page and return (input_loc, output_loc, action_loc)."""
    ...
    return input_locator, output_locator, action_locator
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        # Inject script before every page load to disable JS-based redirect timers.
        # The site uses setTimeout/setInterval to redirect after a few seconds;
        # this blocks any timer whose callback references 'location' or 'redirect'.
            ...
            (function() {
                ...
                window.setTimeout = function(fn, delay) {
                    if (typeof fn === 'function' && delay > 1000) {
                        try {
                            var s = fn.toString();
                            if (s.indexOf('location') !== -1 || s.indexOf('redirect') !== -1) {
                                console.log('[BLOCKED] Redirect setTimeout delay=' + delay);
                                return 0;
                            }
                        } catch(e) {}
                    }
                ...
                window.setInterval = function(fn, delay) {
                    if (typeof fn === 'function') {
                        try {
                            var s = fn.toString();
                            if (s.indexOf('location') !== -1 || s.indexOf('redirect') !== -1) {
                                console.log('[BLOCKED] Redirect setInterval');
                                return 0;
                            }
                        } catch(e) {}
                    }
        ...
        # Block network-level navigations to other domains (catches meta-refresh and
        # any window.location redirect that goes through the network stack).
        _block_external_redirects(page, args.url)

        try:
            input_locator, output_locator, action_locator = _load_page(page, args.url)
        ...
        for row_index in range(header_row + 1, ws.max_row + 1):
            ...
            # If the redirect somehow got through despite the blockers, navigate back.
            try:
                if "pixelssuite.com" not in page.url:
                    print(f"  -> Redirect detected ({page.url[:60]}), navigating back...")
                    input_locator, output_locator, action_locator = _load_page(page, args.url)
            except Exception as nav_e:
                print(f"  -> Recovery navigation failed: {nav_e}")
